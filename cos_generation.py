...
def get_feature_hook(self, input, output):
    ...
    c = 0
    map = output.cpu().numpy()
    for idx in range(map.shape[0]):
        cos_sim = []
        map_select = np.array(map[idx])
        map_select = map_select.reshape(map_select.shape[0], -1)
        for i in range(map_select.shape[0]):
            for j in range(map_select.shape[0]):
                cos_sim.append((np.dot(map_select[i], map_select[j]) / (
                            np.linalg.norm(map_select[i]) * np.linalg.norm(map_select[j]))).tolist())
        cos_sim = np.array(cos_sim)
        cos_sim[np.isnan(cos_sim)] = 0
        cos_sim = cos_sim.reshape(map_select.shape[0], -1)
        c += np.sum(cos_sim, axis=1)
...
def get_feature_hook_densenet(self, input, output):
    ...
    for idx in range(map.shape[0]):
        ...
        map_select = np.array(map[idx])
        map_select = map_select.reshape(map_select.shape[0], -1)
        for i in range((map_select.shape[0]-12),map_select.shape[0]):
            for j in range((map_select.shape[0]-12),map_select.shape[0]):
                cos_sim.append((np.dot(map_select[i], map_select[j]) / (
                            np.linalg.norm(map_select[i]) * np.linalg.norm(map_select[j]))).tolist())
        cos_sim = np.array(cos_sim)
        cos_sim[np.isnan(cos_sim)] = 0
        cos_sim = cos_sim.reshape(12, -1)
        c += np.sum(cos_sim, axis=1)
    c = torch.from_numpy(c)
...
if args.arch=='vgg_16_bn':
    ...
    if not os.path.isdir('cos_conv/' + args.arch+'_limit%d'%(args.limit)):
        os.mkdir('cos_conv/' + args.arch+'_limit%d'%(args.limit))
    ...
    for i in range(3):
        ...
        for j in range(12):
            ...
            np.save('cos_conv/' + args.arch +'_limit%d'%(args.limit) + '/cos_conv%d'%(13*i+j+1)+'.npy', feature_result.numpy())
            feature_result = torch.tensor(0.)
            total = torch.tensor(0.)
        ...
        if i<2:
            ...
            np.save('cos_conv/' + args.arch +'_limit%d'%(args.limit) + '/cos_conv%d' % (13 * (i+1)) + '.npy', feature_result.numpy())
            feature_result = torch.tensor(0.)
            total = torch.tensor(0.)#
    ...
    inference()
    handler.remove()
    np.save('cos_conv/' + args.arch +'_limit%d'%(args.limit) + '/cos_conv%d' % (39) + '.npy', feature_result.numpy())
    feature_result = torch.tensor(0.)
    total = torch.tensor(0.)
    ...
    for idx, cov in enumerate(cov_list):
        ...
        if idx>0:
            for idx1,tp in enumerate(tp_list):
                if idx1==3:
                    np.save('cos_conv/' + args.arch+'_limit%d'%(args.limit) + '/cos_conv%d_'%(idx+1)+tp+'.npy',
            ...
            np.save('cos_conv/' + args.arch+'_limit%d'%(args.limit) + '/cos_conv%d' % (idx + 1) + '.npy',feature_result.numpy())
        feature_result = torch.tensor(0.)
        total = torch.tensor(0.)
    ...
    if not os.path.isdir('rank_conv/' + args.arch+'_limit%d'%(args.limit)):
        os.mkdir('rank_conv/' + args.arch+'_limit%d'%(args.limit))
    np.save('rank_conv/' + args.arch+'_limit%d'%(args.limit) + '/cos_conv%d' % (1) + '.npy', feature_result.numpy())
    feature_result = torch.tensor(0.)
    total = torch.tensor(0.)
    ...
    for i in range(4):
        ...
        for j in range(net.num_blocks[i]):
            ...
            inference()
            handler.remove()
            np.save('rank_conv/' + args.arch+'_limit%d'%(args.limit) + '/cos_conv%d'%(cnt+1)+'.npy',
