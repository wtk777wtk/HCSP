...
if not os.path.isdir(args.job_dir+r'/'+now):
    os.makedirs(args.job_dir+r'/'+now)
job_dir = args.job_dir+r'/'+now

utils.record_config(args,job_dir)
now = datetime.datetime.now().strftime('%Y-%m-%d-%H:%M:%S')
logger = utils.get_logger(os.path.join(job_dir, 'logger'+now+'.log'))
...
def load_vgg_model(model, oristate_dict):
    ...
    subfix = ".npy"
    prefix_cos = args.cos_dir + '/cos_conv'
    for name, module in model.named_modules():
        ...
        if isinstance(module, nn.Conv2d):
            ...
            if orifilter_num != currentfilter_num:
                ...
                logger.info('loading cos from: ' + prefix_cos + str(cov_id) + subfix)
                cos = np.load(prefix_cos + str(cov_id) + subfix)
                select_index_rank = np.argsort(rank)[orifilter_num - currentfilter_num:]  # preserved filter id
                select_index_rank.sort()
                select_index = np.argsort(cos)[orifilter_num - currentfilter_num:]  # preserved filter id
                select_index.sort()
        ...
        for k in range(num):
            for l in range(2):
                ...
                if orifilter_num != currentfilter_num:
                    ...
                    select_index = np.argsort(cos)[orifilter_num - currentfilter_num:]
                    select_index.sort()
        ...
        if isinstance(module, Inception):
            ...
            for weight_index in honey_filter_index:
                ...
                if orifilter_num != currentfilter_num:
                    ...
                    # select_index.sort()
                    logger.info('loading cos from: ' + prefix_cos + str(cov_id) + branch_name + subfix)
                    cos = np.load(prefix_cos + str(cov_id)  + branch_name + subfix)
                    select_index = np.argsort(cos)[orifilter_num - currentfilter_num:]  # preserved filter id
                    select_index.sort()
                else:
                    select_index = list(range(0, orifilter_num))
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module, nn.Conv2d):
            ...
            if orifilter_num != currentfilter_num:
                ...
                logger.info('loading cos from: ' + prefix_cos + str(cov_id) + subfix)
                cos = np.load(prefix_cos + str(cov_id) + subfix)
                select_index = list(np.argsort(cos)[orifilter_num-currentfilter_num:])  # preserved filter id
                select_index.sort()
...
def main():
    ...
    checkpoint_dir = os.path.join(job_dir, 'checkpoint.pth.tar')
    if args.resume:
